...
class PaymentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun makePayment(activity: AppCompatActivity, requestedAmount: String, paymentType: String) {
        viewModelScope.launch {
            ...
            if (installationId.isNullOrEmpty()) {
                ...
                val encryptedRequest = nexoCrypto.encrypt_and_hmac(nexoRequest.toByteArray(), keyId, keyVersion)

                Log.d("PaymentViewModel", "Generated nexo request: $nexoRequest")
                Log.d("PaymentViewModel", "Generated encrypted request: $encryptedRequest")

                createPaymentLink(encryptedRequest, activity)
            }
        }
    }

    @RequiresApi(Build.VERSION_CODES.O)
    private fun createPaymentLink(encryptedRequest: ByteArray, activity: AppCompatActivity) {
        val returnUrl = "merchantscheme://com.merchant.companion/payment"
        val returnUrlEncoded = URLEncoder.encode(returnUrl, Charsets.UTF_8.name())

        val nexoRequestBase64 = Base64.getEncoder().encodeToString(encryptedRequest)
    ...
    fun handleDeepLinkResponse(response: String? = null, error: String? = null) {
        ...
        if (!response.isNullOrEmpty()) {
            ...
            val responseBytes = Base64.getDecoder().decode(response)

            // Decrypt the response
            val decryptedResponse = nexoCrypto.decrypt_and_validate_hmac(responseBytes, keyId, keyVersion)
            Log.d("PaymentViewModel", "Received payment response: ${String(decryptedResponse.packet)}")
    ...
    private fun generateNexoRequest(
        ...
        val maxServiceIdSize = 10
        val serviceId: String = UUID.randomUUID().toString()
        val transactionID: String = "My-Adyen-TTP-Test"
