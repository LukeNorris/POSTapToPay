...
/*class BoardingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun initiateBoarding(activity: AppCompatActivity) {
        ...
        val intent = Intent(Intent.ACTION_VIEW).apply {
            data = Uri.parse("adyenpayments://boarded?returnUrl=$returnUrlEncoded")
        }
    ...
    fun openDeepLink(activity: AppCompatActivity, intent: Intent) {
        try {
            Log.d("BoardingViewModel", "Opening deep link: ${intent.data}")
            if (intent.resolveActivity(activity.packageManager) != null) {
                activity.startActivity(intent)
                Log.d("BoardingViewModel", "Deep link opened successfully")
            } else {
                Log.e("BoardingViewModel", "No activity found to handle deep link: ${intent.data}")
            }
        } catch (e: Exception) {
            Log.e("BoardingViewModel", "Error opening deep link", e)
        }
    }
}*/
