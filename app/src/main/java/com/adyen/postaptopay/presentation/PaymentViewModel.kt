...
/*class PaymentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun makePayment(activity: AppCompatActivity) {
        viewModelScope.launch {
            ...
            if (installationId.isNullOrEmpty()) {
                Log.d(
                    "PaymentViewModel",
                    "Installation ID is empty. Cannot create nexo request without."
                )
            } else {
                Log.d("PaymentViewModel", "Making a payment initiated.")
                val nexoRequest =
                    generateNexoRequest("Luke's cash register", installationId, "EUR", "12")
    ...
    private fun createPaymentLink(nexoRequest: String, activity: AppCompatActivity) {
        ...
        openDeepLink(activity, intent)
    }

    private fun openDeepLink(activity: AppCompatActivity, intent: Intent) {
        try {
            Log.d("PaymentViewModel", "Opening deep link: ${intent.data}")
            if (intent.resolveActivity(activity.packageManager) != null) {
                activity.startActivity(intent)
                Log.d("PaymentViewModel", "Deep link opened successfully")
            } else {
                Log.e("PaymentViewModel", "No activity found to handle deep link: ${intent.data}")
            }
        } catch (e: Exception) {
            Log.e("PaymentViewModel", "Error opening deep link", e)
        }
    }
    ...
    fun makePayment(activity: AppCompatActivity) {
        viewModelScope.launch {
            ...
            if (installationId.isNullOrEmpty()) {
                ...
                Log.d("PaymentViewModel", "Making a payment initiated.")
                val nexoRequest = generateNexoRequest("Luke's cash register", installationId, "EUR", "12")
                Log.d("PaymentViewModel", "Generated nexo request: $nexoRequest")
                createPaymentLink(nexoRequest = nexoRequest, activity)
            }
        }
